...
def main() -> None:
    ...
    while not done:
        ...
        # PV = data.loc[env._time:env._episode_end_time, "pv_production"]
        # W = data.loc[env._time:env._episode_end_time, "wind_production"]
        # C = data.loc[env._time:env._episode_end_time, "consumption"]
        # spot = data.loc[env._time:env._episode_end_time, "spot_market_price"]
        ...
        # C = np.array(C_estim)
        # PV = np.array(PV_estim)

        # N = PV.shape[0]-1
        # print(N)
        action = MPC_step(N, state[3:6],PV[1:], W[1:], C[1:], spot[1:], state[-2] )
        # break
    # i = 0
    # while not done:
